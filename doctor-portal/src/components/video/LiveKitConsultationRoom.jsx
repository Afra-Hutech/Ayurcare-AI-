 ...
 *   fetchToken: (id: string) => Promise<{ token: string, serverUrl: string }>,
 *   onDisconnected?: () => void,
 *   className?: string,
 * }}
 */
const LiveKitConsultationRoom = ({
  appointmentId,
  fetchToken,
  onDisconnected,
  className = '',
  ...
  if (!session) {
    return (
      <div className={`flex flex-col items-center justify-center gap-3 p-10 ${className}`}>
        ...
        <p className="text-sm font-medium text-slate-400">Connecting to secure video…</p>
      </div>
    );
  }
